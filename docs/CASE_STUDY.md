...
- 62 tram stations across 2 lines (T6 with 19 stops, T7 with 43 stops including the Piraeus inbound loop)
...
### Update — June 2026: the M3 / M3_AIR dedup and the suburban explicit-trips upgrade

Two material refinements landed on top of the original refactor.

**M3 / M3_AIR is one physical train, not two.** The first version modeled Line 3 as two scheduling lines that both ran the entire Dim. Theatro → Airport stretch. That double-rendered every airport-bound train on the live map: a city M3 dot and an M3_AIR dot at almost the same position on the Dim. Theatro → DPL section. Operationally, those are the same train — the "airport" run is just a city run that doesn't terminate at DPL and continues to the Airport.

The fix restricted each line to the section it uniquely owns:

- `M3` (city) — 23 stops, 44 min, Dim. Theatro ↔ Doukissis Plakentias only. `seed_station_offsets.py` actively drops Pallini / Kantza / Koropi / Airport from M3 offsets rather than haversine-padding them in.
- `M3_AIR` (airport extension) — 5 stops only: DPL=0 → Pallini=6 → Kantza=8 → Koropi=14 → Airport=19 outbound; reverse inbound. Bands shifted to the DPL-origin arrival time (06:14 outbound; 06:10 inbound since Airport is the inbound origin).

`_display_line_id("M3_AIR") → "M3"` so airport trains surface under the M3 label with `serviceType="airport"`. On the departures board, `_dedupe()` runs a Line-3-specific first pass: if at any minute both an `M3` city outbound row ("Doukissis Plakentias") and an `M3_AIR` outbound row ("Airport") exist, the city row is dropped. Users see one entry labeled "Airport" rather than two redundant rows.

**Suburban moved from headway bands to explicit Hellenic Train trips.** The original suburban seed used coarse hourly placeholder bands (e.g. A1 "every 60 min, 04:00–23:00") because the projector was band-driven. That doesn't match how the Proastiakos actually operates — Hellenic Train publishes a fixed list of named trains with specific HH:MM at each station, not a regular headway.

Migration 0014 added two tables:

| Table | Purpose |
|---|---|
| `scheduled_trips(train_no, line_id, direction, day_type, service_label)` | One row per (train number × day type × direction). |
| `scheduled_trip_stops(train_no, line_id, direction, day_type, station_id, stop_sequence, departure_time)` | Per-station departure time for each trip, with indices on `(line_id, day_type, direction)` and `(station_id, day_type)`. |

`scripts/seed_suburban_trips.py` transcribes 7 Hellenic Train PDFs (valid from 22 Nov 2025, A4 Kiato line from 29 Nov 2025) into 602 trips and 9,832 stop rows covering A1 Piraeus ↔ Airport, A2 Ano Liosia ↔ Airport, A3 Athens ↔ Chalkida (with weekday-only *(1) trains excluded from sat/sun), and A4 Piraeus ↔ Kiato. The projector got a dual path: `SCHEDULED_TRIP_LINES = {A1, A2, A3, A4}` use `_project_scheduled_trip_departures` and `_project_scheduled_trip_active` against the trip rows; metro and tram stay on the existing band path unchanged. Midnight-crossing trains (3203 Airport 23:13 → Tavros 00:10 next day) are detected by a >12h spread in the trip's stop-time set and rebucketed so travel times stay correct. Partial-route trains (3201, 3203 terminate at Tavros) report the correct terminus label by sorting `stop_sequence DESC` for outbound and `ASC` for inbound rather than picking the line's nominal endpoint.

The generator was extended in the same pass: `_line_schedule()` now includes a `trips` array per line so `/api/schedules/{lineId}` ships the full Hellenic Train timetable alongside the existing rules + bands. Clients render trips for suburban without any code change because `/api/live-positions` keeps the same `originDepartureMinute + elapsedMinutes + totalTravelMinutes` shape per train — only the source of those values changed.

API line-id convention for suburban (this is NOT Hellenic Train's PDF heading, which doesn't use line numbers): `A1` = Piraeus ↔ Airport, `A2` = Ano Liosia ↔ Airport, `A3` = Athens ↔ Chalkida, `A4` = Piraeus ↔ Kiato.

**Verified.** M1, M2, T6, T7 band headways and per-station offsets were diffed row-for-row against their STASY PDFs — all match exactly (the apparent "mismatches" in the first diff pass were station-code guessing errors on my side, not data bugs). M3 city + M3_AIR airport extension verified against the Line 3 PDF. Suburban verified by sampling `/api/departures/next` for representative stations and matching them to the PDF trips by train number.
...
| 2026-06-12 (evening) | Petros Dhespollari | Native OASA Tickets/Fares screen across iOS, Android, Web with cards driven by `/api/fares` and Greek titles via parallel scrape (migration 0008). Cross-target euro formatter using integer cents (no `String.format`). STASY status + announcements pipeline: homepage badge ("Κανονική Λειτουργία" / "Trains until HH:MM") + news feed scraped every 4h, served at `/api/announcements` (migration 0009). iOS HomeView gets a service-status pill that falls back to the schedule's last-departure time when the badge is missing; restored "trains until 21:40" component. Map black-screen-after-screenshot regression fixed via `.id()` rebuild key tied to scenePhase and `UIApplication.userDidTakeScreenshotNotification`. Web train icons no longer rendered as Leaflet pins, A2 station icon falls back to sibling-id lookup. Nightly seed-refresh GitHub Action (workflow_dispatch via PAT) wired through `/admin/publish` so admin edits propagate to bundled seeds without manual commits. |
| 2026-06-12 (night) | Petros Dhespollari | STASY announcements filtered to transit-only at the scraper: accent-folded include list (μετρό/τραμ/προαστιακ/σταθμ/δρομολογ + English equivalents) and exclude list dropping HR postings, procurement tenders ("ΠΡΟΣΚΛΗΣΗ ΕΚΔΗΛΩΣΗΣ ΕΝΔΙΑΦΕΡΟΝΤΟΣ"), and cultural events held at metro stations. Greek-to-English translation wired into the pipeline via `deep-translator` GoogleTranslator at ingest time (no on-device translation). Migration 0010 adds `title_en`/`summary_en`/`raw_message_en` columns and `/api/announcements` now emits `titleEn`/`summaryEn`/`rawMessageEn` alongside originals. iOS + KMP read the right field based on `LocalizationManager.language` so the same payload renders in Greek or English without a re-fetch. KMP `STASYAnnouncementService` exposes a `fetchFeed()` flow that yields both status and announcements; `HomeViewModel` mirrors iOS by tracking `serviceStatus` in `HomeUiState`. Home section renamed "Latest from STASY" → "Metro & Tram updates" / "Ενημέρωση Μετρό & Τραμ" and reordered above the Network Overview tiles on iOS and Compose so the operational status is the first thing users see. |
| 2026-06-13 | Petros Dhespollari | OSM-derived rail route geometry for every line: `scripts/snapshot-osm-shapes.py` pulls each line's route relation from Overpass (M1 445858, M2 7963539, M3 445945, T6 3648688, T7 6792078, A1 8467445, A2 8467443, A3 8467442, A4 8467515), stitches track ways in member order with reversal handling, and ships `shapes.json` bundled offline in iOS, Android, KMP and Web. Per-line GeoJSON Features also served at `/line-geometry/{lineId}.geojson` so the Pi can push shape corrections without an app release. Removes the catmullRomSpline-of-station-coords zigzag that made the T7 Piraeus loop look like an X and made the A4 Megara curve cut across the bay. T7 stationCount updated to 43 (loop stops 1-6 followed by spine 7-43). T7_DIM renamed to the canonical dual form "Δημαρχείο / Δημοτικό Θέατρο" so the tram stop can be distinguished from the same-named M3 metro station 150m away. iOS map black-screen regression fixed by wrapping `Map(position:)` in a `Group { ... }.id(mapRebuildKey)` — putting `.id()` directly on Map was silently no-op'd by MapKit-for-SwiftUI's view reuse, leaving the dead Metal layer alive after screenshot or background-to-active scenePhase. Web mobile insight peek bar lifted above iOS Safari's URL bar by switching `100vh` → `100dvh` on the app shell. deploy.sh now excludes `db/` and `backups/` from the rsync so the Pi's persistent SQLite isn't wiped on each deploy. |
| 2026-06-15 | Petros Dhespollari | Major correctness pass plus first-launch onboarding. **iOS structure**: refactored from a flat 100-file `Models/Views/Theme` dump into a staff-engineer layout (`App/`, `Core/{Networking,Persistence,Schedule,Location,Localization,Diagnostics}/`, `DesignSystem/`, `Features/{Timetables,Fares,Onboarding}/`, `Shared/`, `Views/` per-screen folders, `Resources/`). All 27 moved Swift files re-pointed in `project.pbxproj` to SOURCE_ROOT-relative paths so the Navigator hierarchy is purely organizational. **Web station icons**: `/api/icons` was returning empty `{stations:{}, interchanges:{}}` and the JS guard treated "object exists" as authoritative, skipping the bundled manifest fallback so every station rendered as a plain pin. Hydrate from API first, then plug gaps from the manifest. **Alert cards**: dropped the "Show more" / "Read more" duality on iOS + Android — single tap-to-open-source. Web/Desktop already worked this way. **Stale announcements**: STASY keeps evergreen permalinks ("TIME-SCHEDULE OF LINE 3") alive for years. The generator now drops items without an extractable `valid_from` / `valid_until` inside a fresh window (closures: any non-past date; info/warning: 90d window). **M3 / M3_AIR model**: a Line 3 airport-bound train is the same physical train as the city run that continues past Doukissis Plakentias, not a separate dot on the map. Restricted each line to the section it uniquely owns — M3 city = 23 stops, 44 min, Dim. Theatro ↔ DPL (Pallini/Kantza/Koropi/Airport actively dropped from offsets); M3_AIR = 5 stops on the DPL ↔ Airport extension with bands shifted to the airport train's DPL arrival time (06:14 outbound first; 06:10 inbound first, since Airport is the inbound origin). Departures dedup: if a city M3 row and an airport M3_AIR row exist at the same minute outbound, the city row is dropped so the user sees one entry labeled "Airport". `_display_line_id("M3_AIR") → "M3"` so airport trains surface under the M3 label with `serviceType="airport"`. **First-launch onboarding** across iOS / Android / Web: four pages (welcome → live trains → contextual location ask → privacy). iOS uses native SwiftUI with an `ultraThinMaterial` icon disc and animated radial-blob mesh; Android and Web use a shared Compose screen. Android `MainActivity` no longer auto-requests location on launch — `setLocationPermissionRequester` plumbs an `ActivityResultLauncher` so the OS prompt fires from the location step. Gating persists via UserDefaults (iOS) / SharedPreferences (Android) / localStorage (Web). **Suburban explicit timetable** (the big one): the Proastiakos doesn't run on a headway grid — Hellenic Train publishes named train trips (1200, 2202, 3530…) with specific HH:MM at each station. Migration 0014 added `scheduled_trips(train_no, line_id, direction, day_type, service_label)` + `scheduled_trip_stops(train_no, line_id, direction, day_type, station_id, stop_sequence, departure_time)`. `scripts/seed_suburban_trips.py` transcribes 7 PDFs (A1 Pir↔Airport Mon-Fri + Sat-Sun, A2 AnoLiosia↔Airport Mon-Fri + Sat-Sun, A3 Athens↔Chalkida daily with *(1) trains excluded from sat/sun, A4 Pir↔Kiato daily) — 602 trips, 9,832 stop rows. Projector dual-path: `SCHEDULED_TRIP_LINES = {A1,A2,A3,A4}` reads trips directly; metro and tram stay on the existing `frequency_bands` path. Midnight-crossing trips (3203 Airport 23:13 → Tavros 00:10) detected by >12h spread in the stop-time set and rebucketed. Partial-route 3201/3203 terminate at Tavros (not Piraeus) and the destination label correctly reads "Tavros" by picking the smallest stop_sequence for inbound. Generator: `_line_schedule` includes a `trips` array so `/api/schedules/{lineId}` ships the full Hellenic Train timetable. Pre-existing inbound destination label bug also fixed (was returning origin instead of terminus). **Verification**: M1, M2, T6, T7 bands + per-station offsets diffed row-for-row against their STASY PDFs — all match exactly. T7 has a Piraeus extension in the API that the public PDF doesn't yet show (API ahead of source, not a mismatch). API line-id convention documented: A1=Pir↔Airport, A2=AnoLiosia↔Airport, A3=Athens↔Chalkida, A4=Pir↔Kiato. |
